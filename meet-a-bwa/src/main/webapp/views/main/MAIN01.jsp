			...
			<div id="recommendAct" class="mainContent">
				...
				<div id="act_recommendSection">
					...
					<div id="cate_tag_section">
						<div id="unfold_tag">
							<span
								class="tagItem <c:if test = "${checkCategory eq '전체'}">check</c:if>">전체</span>
							<span
								class="tagItem <c:if test = "${checkCategory eq '취미'}">check</c:if>">취미</span>
							<span
								class="tagItem <c:if test = "${checkCategory eq '팬클럽'}">check</c:if>">팬클럽</span>
							<span
								class="tagItem <c:if test = "${checkCategory eq '방송/연예'}">check</c:if>">방송/연예</span>
							<span
								class="tagItem <c:if test = "${checkCategory eq '스포츠/레저'}">check</c:if>">스포츠/레저</span>
							<span
								class="tagItem <c:if test = "${checkCategory eq '게임'}">check</c:if>">게임</span>
							<span
								class="tagItem <c:if test = "${checkCategory eq '만화/애니메이션'}">check</c:if>">만화/애니메이션</span>
							<span
								class="tagItem <c:if test = "${checkCategory eq '맛집/요리'}">check</c:if>">맛집/요리</span>
							<span
								class="tagItem <c:if test = "${checkCategory eq '생활정보/인테리어'}">check</c:if>">생활정보/인테리어</span>
							<span class="tag_plus"> <img src="resources/img/더보기.svg"
								alt="더보기 이미지" id="plusImg"> <img
								src="resources/img/fold.svg" alt="접기 이미지" id="foldImg"
								class="blind">
							</span>
						</div>
						<div id="fold_tag" class="blind">
							<ul id="tagUl">
								<li class="tag_li"><span
									class="tagItem <c:if test = "${checkCategory eq '건강/다이어트'}">check</c:if>">건강/다이어트</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '패션/뷰티'}">check</c:if>">패션/뷰티</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '여행/캠핑'}">check</c:if>">여행/캠핑</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '반려동물/동물'}">check</c:if>">반려동물/동물</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '문화/예술'}">check</c:if>">문화/예술</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '음악'}">check</c:if>">음악</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '어학/외국어'}">check</c:if>">어학/외국어</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '취업/자격증'}">check</c:if>">취업/자격증</span>
								</li>
								<li class="tag_li"><span
									class="tagItem <c:if test = "${checkCategory eq '교육/공부'}">check</c:if>">교육/공부</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '어학/외국어'}">check</c:if>">어학/외국어</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq 'IT/컴퓨터'}">check</c:if>">IT/컴퓨터</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '인문/과학'}">check</c:if>">인문/과학</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '경제/재테크'}">check</c:if>">경제/재테크</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '종교/봉사'}">check</c:if>">종교/봉사</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '일상/이야기'}">check</c:if>">일상/이야기</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '나이/또래모임'}">check</c:if>">나이/또래모임</span>
								</li>
								<li class="tag_li"><span
									class="tagItem <c:if test = "${checkCategory eq '친목/모임'}">check</c:if>">친목/모임</span>
									<span
									class="tagItem <c:if test = "${checkCategory eq '자연/귀농'}">check</c:if>">자연/귀농</span>
								</li>
							</ul>
						</div>
					</div>
					...
					<div id="recommend_list_wrap">
						...
						<c:if test="${a_list.size() == 0}">
							<div class="no-list-wrap">
								<img class="no-list-img"
									src="/meet-a-bwa/resources/img/blue_warning.svg"
									alt='파란 경고 이미지' />
